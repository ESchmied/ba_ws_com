...
//
// This is the length from front to CoG that has a similar radius. test without measuring radius
const double Lf = 2.67;

// Reference velocity
double ref_v = 84 * 0.447; // convert from mph to m/s (für ros2 direkt meter pro sekunde)

// The solver takes all the state variables and actuator
// variables in a singular vector. Thus, we should to establish
// when one variable starts and another ends to make our lifes easier.
// start index for each state
//size_t speichert die max Größe eine theoretisch möglichen Objektes speichern
size_t x_start = 0;
...
public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    size_t i;
    for (i = 0; i < N; ++i) {
      //vars ist ein array oder vector
